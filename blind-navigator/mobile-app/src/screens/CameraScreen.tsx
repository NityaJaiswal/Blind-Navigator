...
export default function CameraScreen({ onOpenSettings }: Props) {
    ...
    const simulateAlarm = () => {
        const random = FAKE_OBJECTS[Math.floor(Math.random() * FAKE_OBJECTS.length)];
        ...
        <Pressable style={styles.container} onPress={handleScreenTap}>
            ...
            {detectedLabel && (
                <View style={styles.overlay}>
                    <Text style={styles.overlayText}>{detectedLabel}</Text>
                    {distance !== null && (
                        <Text style={styles.overlaySubtext}>Distance: {distance} cm</Text>
                    )}
                </View>
            )}

            <TouchableOpacity style={styles.simulateButton} onPress={simulateAlarm}>
                <Text style={styles.buttonText}>Simulate Alarm (test only)</Text>
            </TouchableOpacity>
        </Pressable>
    );
}
...
const styles = StyleSheet.create({
    ...
    overlaySubtext: { color: "#ccc", fontSize: 14, marginTop: 4 },
    simulateButton: {
        position: "absolute",
        bottom: 40,
        left: 20,
